...
import { addProduct, removeProduct,setRestaurantName } from '../utils/cartSlice'; //item to add in cart
...
function RestaurantMenuItemsList({items}) {
    ...
    //console.log("cart items from  RestaurantMenuItemsList.js",cartItems);

    const getItemQuantity = (itemId) => {
        const item = cartItems.find((cartItem) => cartItem.item.card?.info?.id === itemId);
        return item ? item.quantity : 0;
    };
      
    const handleCartItem=(item)=> {
        dispatch(addProduct({item,restaurantName}));
       // console.log("After adding item:", item, "Restaurant:", restaurantName);
        setMessage(`${item.card?.info?.name} from ${restaurantName} has been added to the cart!`);
        setTimeout(()=>{
            setMessage('');
            },3000);
        };
    ...
    const handleRemoveItem=(item)=>{
        dispatch(removeProduct({item,restaurantName}));
    ...
    return (
        <div>
             {message && <div className="alert fixed bottom-0 mx-auto left-1/2 transform -translate-x-1/2 mb-4 px-4 rounded bg-green-500 text-black bg-opacity-8 z-10 h-8 inline-block">{message}</div>} 
            {
                items?.map(item => (
                    <div className='p-2 m-2 flex border-gray-200 border-b-2 justify-between'>
                        <div className='px-4 mx-4 w-8/12'>
                            <div className='py-2'>
                                ...
                                <h5 className='font-bold'> {item.card?.info?.price? `₹ ${item?.card?.info?.price / 100}`: `₹ ${item?.card?.info?.defaultPrice / 100}`}</h5>
                            </div>
